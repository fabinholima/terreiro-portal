...
export default class PagesController {
  private async renderPage(slug: keyof typeof defaults, { view, response }: HttpContext) {
    const data = defaults[slug]
    const page = await InstitutionalPage.firstOrCreate(
      { slug },
      {
        slug,
        title: data.title,
  ...
  async about(ctx: HttpContext) {
    return this.renderPage('o-terreiro', ctx)
  }

  async history(ctx: HttpContext) {
    return this.renderPage('nossa-historia', ctx)
  }

  async umbanda(ctx: HttpContext) {
    return this.renderPage('umbanda', ctx)
  }

  async contact(ctx: HttpContext) {
    return this.renderPage('contato', ctx)
  }
}
